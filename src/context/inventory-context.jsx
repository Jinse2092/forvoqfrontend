...
export const InventoryProvider = ({ children }) => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const results = await Promise.all(endpoints.map(ep => fetch(`http://localhost:4000/api/${ep}`).then(res => {
      ...
      inventoryWithPacked.forEach(async (inv) => {
        try {
          ...
          if (Number(existingPacked) !== Number(inv.packedQuantity)) {
            await fetch(`http://localhost:4000/api/inventory/${inv.id}`, {
              method: 'PATCH',
  ...
  // --- Inventory CRUD ---
   ...
   const deleteInventoryItem = async (id) => {
     ...
     try {
      const response = await fetch(`http://localhost:4000/api/inventory/${id}`, { method: 'DELETE' });
  ...
  const updateOrder = async (orderId, updatedFields) => {
    try {
      ...
      try {
        const debugResp = await fetch(`http://localhost:4000/api/orders-debug/${orderId}`);
        if (debugResp.ok) {
  ...
  const removeOrder = async (orderId) => {
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/orders/${orderId}`, { method: 'DELETE', headers });
  ...
  // --- Inbound Management ---
   const addInboundRequest = async (inboundData) => {
     ...
     try {
      const response = await fetch('http://localhost:4000/api/inbounds', {
         method: 'POST',
         headers: { 'Content-Type': 'application/json' },
   ...
   const receiveInbound = async (inboundId) => {
     ...
     try {
      const response = await fetch(`http://localhost:4000/api/inbounds/${inboundId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const addPickupLocation = async (location) => {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/savedPickupLocations', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const updatePickupLocation = async (id, updatedLocation) => {
    try {
      const response = await fetch(`http://localhost:4000/api/savedPickupLocations/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
