  ...
  try {
    ...
    for (const ent of entries) {
      ...
      if (order.address) {
        ...
        for (const ln of lines) { doc.text(ln, margin, y); y += 14 * FONT_SCALE; }
      }
      const cityLine = `${order.city || ''}${order.city ? ', ' : ''}${order.state || ''}${order.pincode ? ' - ' + order.pincode : ''}`.trim();
      if (cityLine) { doc.text(cityLine, margin, y); y += 16 * FONT_SCALE; }
      ...
      for (const it of items) {
        ...
        doc.text(lines, margin + 8 * FONT_SCALE, y);
        y += lines.length * baseLineHeight + 6 * FONT_SCALE;
        if (y > pageHeight - margin - 60 * FONT_SCALE) { doc.addPage(); y = margin + 20 * FONT_SCALE; }
      }

      doc.setFont(undefined, 'bold');
      doc.text(`Total items: ${totalQty}`, pageWidth - margin - 40 * FONT_SCALE, y, { align: 'right' });

      // Footer
      const footerY = pageHeight - margin;
      doc.setFontSize(9 * FONT_SCALE);
      doc.setFont(undefined, 'normal');
      doc.text(`Manifest generated by ${merchant.companyName || merchant.name || 'Merchant'} · ${new Date().toLocaleString()}`, margin, footerY);
    }

    const blob = doc.output('blob');
    // Directly download the PDF (do not open preview)
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
