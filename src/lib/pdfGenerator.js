import { jsPDF } from 'jspdf';

// Generate a basic shipping label PDF, open a preview window and trigger print.
// Returns a Promise that resolves with the generated Blob.
export async function generateShippingLabelPDF(order = {}, merchant = {}) {
  try {
    // create document in points for predictable sizing
    const doc = new jsPDF({ unit: 'pt', format: 'a4' });
    const margin = 36; // 0.5in margins
    const pageWidth = doc.internal.pageSize.getWidth();

    // Header
    doc.setFontSize(14);
    doc.setFont(undefined, 'bold');
    doc.text(merchant.companyName || 'Merchant', margin, 50);
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
    if (merchant.address) doc.text(String(merchant.address), margin, 64);
    if (merchant.phone) doc.text(`Phone: ${merchant.phone}`, margin, 78);

    // Order meta
    const metaX = pageWidth - margin - 220;
    doc.setFont(undefined, 'bold');
    doc.text(`ORDER: ${order.id || order.name || ''}`, metaX, 50);
    doc.setFont(undefined, 'normal');
    const created = order.created_at || order.createdAt || order.date || '';
    const createdStr = created ? new Date(created).toLocaleString() : '';
    if (createdStr) doc.text(`Date: ${createdStr}`, metaX, 66);
    if (order.tracking_number) doc.text(`Tracking: ${order.tracking_number}`, metaX, 82);

    // Barcode (CODE128) for order id — try to use JsBarcode if installed, fallback to plain text
    if (order.id) {
      try {
        const canvas = document.createElement('canvas');
        canvas.width = 400;
        canvas.height = 80;
        try {
          const JsBarcodeModule = await import('jsbarcode');
          const JsBarcode = JsBarcodeModule && (JsBarcodeModule.default || JsBarcodeModule);
          if (typeof JsBarcode === 'function') {
            JsBarcode(canvas, String(order.id), { format: 'CODE128', displayValue: false, height: 60, margin: 0, width: 1 });
            const barcodeDataUrl = canvas.toDataURL('image/png');
            const bw = 180;
            const bh = 50;
            const bx = metaX; // position near the meta area
            const by = 88; // slightly below the meta lines
            try { doc.addImage(barcodeDataUrl, 'PNG', bx, by, bw, bh); } catch (e) { /* ignore image add errors */ }
          } else {
            // fallback when JsBarcode is not exportable as function
            doc.setFontSize(10);
            doc.setFont(undefined, 'normal');
            doc.text(String(order.id), metaX, 92);
          }
        } catch (e) {
          // dynamic import failed or JsBarcode not installed — fallback to text
          doc.setFontSize(10);
          doc.setFont(undefined, 'normal');
          doc.text(String(order.id), metaX, 92);
        }
      } catch (e) {
        // ignore barcode generation errors
      }
    }

    // From / To blocks
    let y = 110;
    doc.setFont(undefined, 'bold');
    doc.text('FROM', margin, y);
    doc.text('TO', metaX, y);
    y += 14;

    doc.setFont(undefined, 'normal');
    // FROM details (merchant)
    const fromLines = [];
    if (merchant.companyName) fromLines.push(merchant.companyName);
    if (merchant.id) fromLines.push(`ID: ${merchant.id}`);
    if (merchant.address) fromLines.push(merchant.address);
    if (merchant.city || merchant.state || merchant.pincode) fromLines.push(`${merchant.city || ''}${merchant.city ? ', ' : ''}${merchant.state || ''} ${merchant.pincode || ''}`.trim());
    if (merchant.phone) fromLines.push(`Phone: ${merchant.phone}`);
    doc.text(fromLines, margin, y);

    // TO details (customer)
    const toLines = [];
    if (order.customerName) toLines.push(order.customerName);
    if (order.address) toLines.push(order.address);
    const cityLine = `${order.city || ''}${order.city ? ', ' : ''}${order.state || ''}${order.pincode ? ' — PIN: ' + order.pincode : ''}`.trim();
    if (cityLine) toLines.push(cityLine);
    if (order.phone) toLines.push(`Phone: ${order.phone}`);
    doc.text(toLines, metaX, y);

    // Items table
    const items = Array.isArray(order.items) ? order.items : (order.line_items || order.order_items || []);
    const tableY = y + Math.max(fromLines.length, toLines.length) * 12 + 18;
    doc.setFont(undefined, 'bold');
    doc.text('Items', margin, tableY);
    doc.setFont(undefined, 'normal');

    const startY = tableY + 12;
    const col1 = margin;
    const col2 = pageWidth - margin - 120;
    const col3 = pageWidth - margin - 40;

    // Header row
    doc.setFontSize(10);
    doc.text('Description', col1, startY);
    doc.text('SKU', col2, startY);
    doc.text('Qty', col3, startY, { align: 'right' });

    let rowY = startY + 12;
    const maxRowWidth = col2 - col1 - 8;
    let totalQty = 0;
    for (const it of items) {
      const title = String(it.title || it.name || it.productName || 'Item');
      const sku = String(it.sku || it.productId || '');
      const qty = Number(it.quantity || it.qty || it.count || 0);
      totalQty += qty;

      // wrap description
      const descLines = doc.splitTextToSize(title, maxRowWidth);
      doc.text(descLines, col1, rowY);
      // SKU and qty on same row as first desc line
      doc.text(sku, col2, rowY);
      doc.text(String(qty), col3, rowY, { align: 'right' });
      rowY += descLines.length * 12 + 6;
      // add page if near bottom
      if (rowY > doc.internal.pageSize.getHeight() - margin - 60) {
        doc.addPage();
        rowY = margin + 20;
      }
    }

    // Totals
    doc.setFont(undefined, 'bold');
    doc.text(`Total items: ${totalQty}`, col3, rowY, { align: 'right' });

    // Courier / Carrier (render near footer, right side)
    const pageHeight = doc.internal.pageSize.getHeight();
    const footerY = pageHeight - margin;
    // determine courier from a few possible fields on the order
    const courierRaw = order.courier || order.carrier || (order.shipping && (order.shipping.name || order.shipping.carrier)) || order.shipping_method || order.shipping_method_title || order.shipping_title || '';
    const courierText = String(courierRaw || '').trim();
    if (courierText) {
      doc.setFontSize(16);
      doc.setFont(undefined, 'bold');
      // Draw courier text at bottom-right, slightly above the footer
      const courierX = pageWidth - margin - 10;
      const courierY = footerY - 24; // 24pt above footer line
      doc.text(courierText.toUpperCase(), courierX, courierY, { align: 'right' });
    }

    // Footer
    doc.setFontSize(9);
    doc.setFont(undefined, 'normal');
    doc.text(`Print generated by ${merchant.companyName || 'Merchant'} · Generated on ${new Date().toLocaleString()}`, margin, footerY);

    // Output as blob and open in preview window
    const blob = doc.output('blob');
    const url = URL.createObjectURL(blob);

    // Try to open a new window/tab with embedded PDF for preview and print
    const w = window.open('', '_blank');
    if (w) {
      try {
        w.document.title = `Shipping Label - ${order.id || ''}`;
        w.document.body.style.margin = '0';
        const embed = w.document.createElement('embed');
        embed.src = url;
        embed.type = 'application/pdf';
        embed.width = '100%';
        embed.height = '100%';
        w.document.body.appendChild(embed);
        // attempt to trigger print after a short delay
        setTimeout(() => {
          try { w.focus(); w.print(); } catch (e) { /* ignore print errors */ }
        }, 600);
      } catch (e) {
        // fallback: navigate window to blob URL
        try { w.location.href = url; } catch (e2) { console.error('Could not write preview window', e2); }
      }
    } else {
      // popup blocked: open in current window as fallback
      const a = document.createElement('a');
      a.href = url;
      a.target = '_blank';
      a.rel = 'noopener noreferrer';
      a.click();
    }

    // revoke the object URL after some time
    setTimeout(() => URL.revokeObjectURL(url), 60 * 1000);

    return blob;
  } catch (err) {
    console.error('generateShippingLabelPDF error', err);
    throw err;
  }
}

// Generate a manifest PDF for multiple orders. Each entry should be an object
// with shape: { order: {...}, merchant: {...} }
export async function generateManifestPDF(entries = []) {
  try {
    const doc = new jsPDF({ unit: 'pt', format: 'a4' });
    const margin = 36;
    const pageWidth = doc.internal.pageSize.getWidth();
    const pageHeight = doc.internal.pageSize.getHeight();

    let first = true;
    for (const ent of entries) {
      const order = ent.order || {};
      const merchant = ent.merchant || {};

      if (!first) doc.addPage();
      first = false;

      doc.setFontSize(14);
      doc.setFont(undefined, 'bold');
      doc.text(merchant.companyName || merchant.name || 'Merchant', margin, 50);
      doc.setFontSize(11);
      doc.setFont(undefined, 'normal');
      if (merchant.address) doc.text(String(merchant.address), margin, 66);

      // Order header
      doc.setFontSize(12);
      doc.setFont(undefined, 'bold');
      doc.text(`Order: ${order.id || order.name || ''}`, pageWidth - margin - 220, 50);
      doc.setFont(undefined, 'normal');
      const created = order.created_at || order.createdAt || order.date || '';
      const createdStr = created ? new Date(created).toLocaleString() : '';
      if (createdStr) doc.text(`Date: ${createdStr}`, pageWidth - margin - 220, 66);

      // Customer
      let y = 100;
      doc.setFont(undefined, 'bold');
      doc.text('Customer', margin, y);
      doc.setFont(undefined, 'normal');
      y += 16;
      if (order.customerName) doc.text(String(order.customerName), margin, y);
      y += 14;
      if (order.address) {
        const lines = String(order.address).split('\n').map(s => s.trim()).filter(Boolean);
        for (const ln of lines) { doc.text(ln, margin, y); y += 14; }
      }
      const cityLine = `${order.city || ''}${order.city ? ', ' : ''}${order.state || ''}${order.pincode ? ' - ' + order.pincode : ''}`.trim();
      if (cityLine) { doc.text(cityLine, margin, y); y += 16; }

      // Items
      doc.setFont(undefined, 'bold');
      doc.text('Items', margin, y);
      y += 14;
      doc.setFont(undefined, 'normal');
      const items = Array.isArray(order.items) ? order.items : (order.line_items || order.order_items || []);
      let totalQty = 0;
      for (const it of items) {
        const title = String(it.title || it.name || it.productName || 'Item');
        const qty = Number(it.quantity || it.qty || it.count || 0);
        totalQty += qty;
        // wrap long lines
        const maxWidth = pageWidth - margin * 2;
        const lines = doc.splitTextToSize(`${title} (Qty: ${qty})`, maxWidth - 20);
        doc.text(lines, margin + 8, y);
        y += lines.length * 12 + 6;
        if (y > pageHeight - margin - 60) { doc.addPage(); y = margin + 20; }
      }

      doc.setFont(undefined, 'bold');
      doc.text(`Total items: ${totalQty}`, pageWidth - margin - 40, y, { align: 'right' });

      // Footer
      const footerY = pageHeight - margin;
      doc.setFontSize(9);
      doc.setFont(undefined, 'normal');
      doc.text(`Manifest generated by ${merchant.companyName || merchant.name || 'Merchant'} · ${new Date().toLocaleString()}`, margin, footerY);
    }

    const blob = doc.output('blob');
    // try to open in new window for preview
    const url = URL.createObjectURL(blob);
    const w = window.open('', '_blank');
    if (w) {
      try {
        w.document.title = `Manifest`;
        w.document.body.style.margin = '0';
        const embed = w.document.createElement('embed');
        embed.src = url;
        embed.type = 'application/pdf';
        embed.width = '100%';
        embed.height = '100%';
        w.document.body.appendChild(embed);
      } catch (e) {
        try { w.location.href = url; } catch (e2) { console.error('Could not write preview window', e2); }
      }
    } else {
      const a = document.createElement('a');
      a.href = url;
      a.target = '_blank';
      a.rel = 'noopener noreferrer';
      a.click();
    }

    setTimeout(() => URL.revokeObjectURL(url), 60 * 1000);
    return blob;
  } catch (err) {
    console.error('generateManifestPDF error', err);
    throw err;
  }
}
